#!/usr/bin/env bash
# Management script for <del>stashbot</del> grid-webservices kubernetes processes
# https://github.com/wikimedia/stashbot/blob/master/bin/stashbot.sh

set -e

DEPLOYMENT=grid-webservices
POD_NAME=grid-webservices

TOOL_DIR=/data/project/grid-webservices/www/python/src
...
case "$1" in
    start)
        echo "Starting grid-webservices k8s deployment..."
    ...
    stop)
        ...
        ;;
    restart)
        echo "Restarting grid-webservices pod..."
